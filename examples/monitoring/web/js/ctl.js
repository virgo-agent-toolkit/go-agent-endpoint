...
Ctl.prototype.poll = function(clientID) {
  d3.json('/data?clientID=' + clientID, function(data) {
    ...
    this.agents[data.AgentName][data.Name]
    .data[this.agents[data.AgentName][data.Name].next] = {
      time: new Date().getTime(),
      value: data.Data
    };
    this.agents[data.AgentName][data.Name].next = index_plus(
      this.agents[data.AgentName][data.Name].next, this.cache_num, 1);
...
function index_minus(original, count, offset) {
  ...
  return (original - offset) >= 0 ?
    (original - offset) : (original - offset + count);
}
...
Ctl.prototype.get_metric = function(agent, metric_name) {
  return this.context.metric(function(start, stop, step, callback) {
    ...
    var ret = [];
    for (var i = index_minus(last, this.cache_num, num);
      ret.length < num;
      i = index_plus(i, this.cache_num, 1)) {
...
Ctl.prototype.new_agent_graph = function(agentName) {
  ...
  for (var i = 0; i < this.metric_names.length; i++) {
    cubism_metrics.push(
      this.get_metric(this.agents[agentName], this.metric_names[i]));
  }
